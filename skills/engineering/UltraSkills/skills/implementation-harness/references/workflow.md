...
- If routing cannot be resolved safely, report a `NON_BLOCKING_STOP` and request the smallest routing decision needed.
...
3. Stay in orchestrator mode: do not implement inside the parent session. Spawn delegated implementer worker/subagent owner(s) for the approved file zones even when direct manual execution in the parent session would be faster or more convenient.
   - if the required implementer worker/subagent path is unavailable, fails to start, or cannot be used, report a `NON_BLOCKING_STOP` and request help instead of implementing manually in the parent session; resume the same stage after resolution
...
- Required role material must be loaded before acceptance. If required material cannot be loaded or loaded role material's additional, final-answer, or output requirements cannot be satisfied, report a `NON_BLOCKING_STOP` for that owner and do not hand the slice to review as complete.
- Parent-session convenience is never a valid reason to bypass delegated implementation when this harness applies.
- Delegate failure or unavailability is not a license to bypass the harness; if required implementer delegation cannot run, report a `NON_BLOCKING_STOP` and request help.
...
- If execution hits a concrete blocker, scope expansion is required, a contradiction appears, or an implementation-critical fact is still missing, report a `NON_BLOCKING_STOP` with the smallest concrete help request. Resume the same task after resolution.
