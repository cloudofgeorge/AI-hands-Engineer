...
def main() -> None:
    ...
    for pattern, label in [
        ...
        (r"non-infrastructure-mutating", "plan mutation boundary"),
        (r"(?:can|may).*acquire.*lock", "plan lock-acquisition boundary"),
        (r"-refresh-only", "refresh-only diagnosis path"),
        (r"-refresh=false", "stale-plan guardrail"),
        (r"-invoke", "invoked-action plan guardrail"),
