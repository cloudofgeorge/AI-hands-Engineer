#!/usr/bin/env python3
"""Structural contract for the agent-neutral observability safety skill."""
...
SKILL = ROOT / "skills/engineering/domains/infrastructure/observability/SKILL.md"
...
def main() -> None:
    ...
    for pattern, label in [
        ...
        (r"SLI", "service-level indicator boundary"),
        (r"SLO", "service-level objective boundary"),
        (r"silence", "alert-silence guardrail"),
        (r"time-box", "time-boxed exception rule"),
        (r"promtool check rules", "rule validation command"),
        (r"cardinality", "metric-cardinality consideration"),
        (r"traces", "multi-signal diagnosis"),
        (r"logs", "multi-signal diagnosis"),
        (r"rollback", "rollback requirement"),
        (r"prometheus\.io", "official Prometheus documentation link"),
    ]:
        require(text, pattern, label)

    readme = README.read_text(encoding="utf-8")
    require(readme, r"\[observability\]\(\./domains/infrastructure/observability/SKILL\.md\)", "engineering routing entry")

    skills_index = SKILLS_INDEX.read_text(encoding="utf-8")
    count_match = re.search(r"\[Engineering\]\(\./engineering/README\.md\) \| (\d+) \|", skills_index)
    if not count_match:
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL: {error}")
        sys.exit(1)
    print("PASS: Observability safety skill contract")
